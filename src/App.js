...
function TaskManager() {
  ...
  return (
    <div>
      <h1>ADITYA'S Task Manager</h1>
      <p>Welcome, {user}</p>
      <input ref={taskInputRef} type="text" placeholder="Add new task" />
      <button  style={{ backgroundColor: 'blue', color: 'white' }}  onClick={addTask}>Add Task</button>
      <ul>
        {tasks.map((task, index) => (
          <li key={index}>
            ...
            <button style={{ backgroundColor: 'blue', color: 'white' }} onClick={() => deleteTask(index)}>Delete</button>
          </li>
        ))}
      </ul>
